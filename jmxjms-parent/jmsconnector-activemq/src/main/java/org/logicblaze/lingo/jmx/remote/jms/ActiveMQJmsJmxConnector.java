    ...
    public void connect(Map<String, ?> env) throws IOException{
        if(!connected){
            ...
            if(destinationName==null){
                destinationName=ActiveMQJmsJmxConnectorSupport.DEFAULT_DESTINATION_PREFIX+destinationGroupName+"."
            ...
            try{
                ...
                requestor=(MultiplexingRequestor) MultiplexingRequestor.newInstance(fac,new JmsProducerConfig(),
                                destination);
                proxy.setRequestor(requestor);
                proxy.setServiceInterface(MBeanJmsServerConnection.class);
                proxy.afterPropertiesSet();
                sendConnectionNotificationOpened();
                client=new MBeanJmsServerConnectionClient((MBeanJmsServerConnection) proxy.getObject(),requestor
                                .getConnection());
