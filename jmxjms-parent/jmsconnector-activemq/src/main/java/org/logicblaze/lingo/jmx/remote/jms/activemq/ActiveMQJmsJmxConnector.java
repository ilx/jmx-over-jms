...
import org.logicblaze.lingo.jmx.remote.jms.MBeanJmsServerConnectionClient;
 ...
 *
 * <p>
 * A newly-created object of this type is unconnected. Its {@link#connect connect} method must be called before it can
 ...
 * @since.unbundled 1.0
 */
public class ActiveMQJmsJmxConnector implements JmsJmxConnector {
	...
	private String destinationGroupName = ActiveMQJmsJmxConnectorSupport.MBEAN_GROUP_NAME;
	private String destinationServerName = ActiveMQJmsJmxConnectorSupport.MBEAN_SERVER_NAME;
	private URI jmsURL;
	private MultiplexingRequestor requestor;
	private JmsProxyFactoryBean<MBeanServerConnection> proxy;
	 ...
	 * @throws IOException
	 */
	public ActiveMQJmsJmxConnector(Map env, JMXServiceURL url) throws IOException {
		this.env = env;
		this.jmsURL = ActiveMQJmsJmxConnectorSupport.getProviderURL(url);
	 ...
	 *
	 * @param env
	 *            the properties of the connection. Properties in this map override properties in the map specified when
	...
	public void connect(Map<String, ?> env) throws IOException {
		if (!connected) {
			proxy = new JmsProxyFactoryBean<MBeanServerConnection>();
			ConnectionFactory fac = new ActiveMQConnectionFactory(jmsURL);
			if (destinationName == null) {
				destinationName = ActiveMQJmsJmxConnectorSupport.DEFAULT_DESTINATION_PREFIX + destinationGroupName + "." + destinationServerName;
			}
			try {
				...
				requestor = (MultiplexingRequestor) MultiplexingRequestor.newInstance(fac, new JmsProducerConfig(), destination);
				proxy.setRequestor(requestor);
				proxy.setServiceInterface(MBeanJmsServerConnection.class);
				proxy.afterPropertiesSet();
				sendConnectionNotificationOpened();
				MBeanJmsServerConnection jmsServerConnection = (MBeanJmsServerConnection) proxy.getObject();
				client = new MBeanJmsServerConnectionClient(jmsServerConnection, requestor.getConnection());
	 ...
	 *            the <code>Subject</code> on behalf of which requests will be performed. Can be null, in which case
	 ...
	 *
	 * @exception IOException
	 *                if the connection cannot be closed cleanly. If this exception is thrown, it is not known whether
	 ...
	 * @param filter
	 *            a filter to select which notifications are to be delivered to the listener, or null if all
	 ...
	 *                if the listener is not registered with this <code>JMXConnector</code>.
	 *
	 ...
	 *
	 * @exception ListenerNotFoundException
	 *                if the listener is not registered with this <code>JMXConnector</code>, or is not registered
	...
	public String getConnectionId() {
		try {
			return requestor.getConnection().getClientID();
		} catch (JMSException e) {
	...
	private void sendConnectionNotificationOpened() {
		JMXConnectionNotification notification = new JMXConnectionNotification(JMXConnectionNotification.OPENED, this, getConnectionId(),
				notificationNumber.incrementAndGet(), "Connection opened", null);
		connectionNotifier.sendNotification(notification);
	}

	private void sendConnectionNotificationClosed() {
		JMXConnectionNotification notification = new JMXConnectionNotification(JMXConnectionNotification.CLOSED, this, getConnectionId(),
				notificationNumber.incrementAndGet(), "Connection closed", null);
		connectionNotifier.sendNotification(notification);
	}

	private void sendConnectionNotificationFailed(String message) {
		JMXConnectionNotification notification = new JMXConnectionNotification(JMXConnectionNotification.FAILED, this, getConnectionId(),
				notificationNumber.incrementAndGet(), message, null);
		connectionNotifier.sendNotification(notification);
	}
